    ...
    <main className="flex-1 p-4 md:p-6">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6 transition-all duration-300 ${selectedPost ? 'blur-[1px]' : ''}`}>
        {posts.map((post) => (
          <div
            key={post.id}
            className="group relative rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 bg-gray-800 cursor-pointer"
            onClick={() => navigate(`/post/${post.id}`)}
          >
            {/* Show first photo as main image */}
            <img
              src={post.photos[0]}
              alt={post.title}
            ...
            {/* Photo Grid Preview - Show multiple photos if available */}
            {post.photos.length > 1 && (
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900 via-gray-900/80 to-transparent p-2">
                <div className="flex gap-1 overflow-x-auto">
                  {post.photos.slice(0, 4).map((photo, photoIdx) => (
                    <div key={photoIdx} className="flex-shrink-0">
                      <img
                        src={photo}
                        alt={`${post.title} photo ${photoIdx + 1}`}
            ...
            {/* Hover Overlay */}
            <div className="absolute inset-0 bg-gray-900 bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-300 flex items-end">
              <div className="w-full p-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="bg-gray-800 bg-opacity-80 rounded-lg p-2 text-center">
                  <p className="text-sm font-medium">{post.title}</p>
                  <p className="text-xs opacity-80">{post.photos.length} photo{post.photos.length !== 1 ? 's' : ''}</p>
                </div>
              </div>
            </div>
          </div>
